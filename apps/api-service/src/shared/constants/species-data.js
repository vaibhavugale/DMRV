...
exports.SPECIES_REGISTRY = void 0;
exports.getSpeciesDecisionTree = getSpeciesDecisionTree;
exports.getFamilies = getFamilies;
exports.getGeneraForFamily = getGeneraForFamily;
exports.getSpeciesForGenus = getSpeciesForGenus;
exports.findSpecies = findSpecies;
exports.SPECIES_REGISTRY = [
...
function getSpeciesDecisionTree() {
    const tree = {};
    for (const sp of exports.SPECIES_REGISTRY) {
        if (!tree[sp.family])
            tree[sp.family] = {};
        if (!tree[sp.family][sp.genus])
            tree[sp.family][sp.genus] = [];
...
function getFamilies() {
    return [...new Set(exports.SPECIES_REGISTRY.map(s => s.family))].sort();
}
// Get genera for a family
function getGeneraForFamily(family) {
    return [...new Set(exports.SPECIES_REGISTRY.filter(s => s.family === family).map(s => s.genus))].sort();
}
// Get species for a genus
function getSpeciesForGenus(genus) {
    return exports.SPECIES_REGISTRY.filter(s => s.genus === genus);
}
// Find species by scientific name
function findSpecies(scientificName) {
    return exports.SPECIES_REGISTRY.find(s => s.scientificName === scientificName);
}
